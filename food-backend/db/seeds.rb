...
require 'json'
  ...
  list_array.each do |list_item|
    ...
    # place_doc_link = open("http://www.grubstreet.com/bestofnewyork/the-absolute-best-poutine-in-nyc.html")
    ...
    place_info = nokogiri_doc.css(".clay-paragraph").each_with_index do |place,index|
      ...
      if index == 0
        ...
        if address_array.length == 4
          ...
          address = [address_array[0], "New York"].join(" ")
        else
          address = address_array.join(" ")
          address = ""
        end
        place_hash = {name: place.children[0].children.text, address: address}
